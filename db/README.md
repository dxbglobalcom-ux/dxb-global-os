...
| The dashboard | `pnpm dashboard` (dev) / `pnpm dashboard:start` — `scripts/dashboard.sh` does the same mapping the daemons do. Next.js reads env files from `apps/dashboard/`, not from the repository root, so the wrapper is the only thing that gives the voice-call route an address. **Starting it any other way is refused:** the wrapper stamps `DXB_DASHBOARD_LAUNCHER` and `apps/dashboard/src/instrumentation.ts` stops the server at startup without it (`exit 2`), because on 2026-08-24 a dashboard started by hand had been serving for 6h50m with no address at all. It listens on **127.0.0.1** by default — the same day it was found answering on the home network at `192.168.178.44:3000`; set `DXB_DASHBOARD_HOST` to change that deliberately, and `DXB_DASHBOARD_PORT` for the port. Held by `tests/ops/dashboard-launcher.host.test.ts`. |
