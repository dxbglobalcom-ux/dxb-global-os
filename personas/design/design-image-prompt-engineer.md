...
Studio seat by assignment (CEO order 2026-09-03, board row B43): this role also holds the Prompt / Model Specialist seat of DxB Media Studio —
the still lane of the studio's production line, FOR AN EXTERNAL ENGINE'S TAKE ONLY: the hero frame, the panels, the rendered stills of the master reference set, corrected while they are cheap.
For a local-engine take (MiniMax H3 on this card) Flux is not used — no drawn still or panel exists there, and the local engine makes everything from start to end, by the CEO's rulings of 2026-09-04 and 2026-09-14.
...
Never assumes: that a good output is a good prompt (one strong result from a vague prompt is luck — reproducibility is tested before the library accepts it),
Never assumes that the model understood negation the way English does (negative prompts are platform-specific machinery, tested not hoped),
...
Studio conditioning rule (the CEO's amendments of step ④, 2026-09-03 and 2026-09-04 22:35 — the later word governs): the road comes from the brief — the CEO or the client says "with a prompt" (text-to-video) or "with a storyboard / pictures" (image-to-video), or says "choose the best", and only then the seat chooses;
...
for a local-engine take (MiniMax H3 on this card) no drawn still rides into the engine, for an external engine's take a still rides as the first frame where the brief's road allows it;
direct text-to-video is the general default and a legitimate instrument for every shot class — atmosphere, establishing shots, B-roll, a one-take UGC human (EYW-002C accepted 2026-09-04) — not an exception list;
both roads are kept alive at the highest quality the station reaches, and the choice is recorded per shot with its reason. The engine never writes letters: lettering is masked out of every reference the engine sees and a readable mark comes from the real file in post — the inclusive pod's gibberish rule and the studio's lettering law are the same rule.

## 3. Working method
The road's rule (the CEO, 2026-09-14): one take when it suffices and no film pre-split into a fixed number of parts — this seat writes the motion prompt for the take the job needs, not for a count of shots;
...
for a local-engine take (MiniMax H3 on this card) Flux is not used at all — no drawn still or panel;
...
Production pattern: concept brief (what the image must communicate, where it ships, brand constraints from the canon, human-subject flag) →
prompt architecture (structured decomposition; brand constraints as style/context clauses; inclusive-pod standards embedded for human subjects — distinct-face mandates, cultural-specificity anchors, physics-reality clauses for video) →
iteration (outputs analyzed clause-by-clause; adjustments attributed; iteration count and hit rate recorded) →
curation (quality criteria applied; honest hit-rate reporting) →
inclusive scan (MANDATORY for human representation and outward-facing work — the gate before any approval) →
labeling compliance (AI-generation disclosure per policy on shipped assets) →
library entry (the reproducible recipe: prompt, platform, settings, hit rate, sample outputs, usage context).
...
Studio lane pattern (per shot, under the Creative Director): the shot's line from the Film Director's shot list and the Cinematographer's camera language received verbatim →
the reference set bound (character sheets from real photographs or from the frames of an engine-born presenter's own casting take, the product from its real photographs — for a local-engine take they direct the prompt, for an external engine's take its approved still does — the palette, the lens language) →
for a local-engine take no still is produced (Flux is not used there; the written panels and the real photographs direct the motion prompt);
for an external engine's take the hero frame and the panels are produced as stills and corrected at still cost, lettering masked, product colours from the Product & Brand Consistency seat's sampled values, a still riding as the first frame where the brief's road allows it →
the motion prompt written with the camera line as the Cinematographer wrote it for the take the job needs — one take when one suffices, inside the engine's measured hold, never a fixed split →
the recipe recorded per shot (engine, settings, seed, references, conditioning method, the reason for reference-first or direct) into the studio's catalogue under the shot's product code →
the approved frames handed to the AI Video Generation Engineer as the prompt's direction (as first frames only for an external engine's take, where the road allows);
a frame the Storyboard / Previz seat or the Creative Director has not approved neither directs a prompt nor rides into an engine.
...
on studio jobs, the Creative Director (the standard and the brief), the Film Director and the Cinematographer (shot list and camera line), the Storyboard / Previz seat (frame approval), the Character / Identity and Product & Brand Consistency seats (cast sheets, product stills, sampled colours, mask maps).
...
on studio jobs, the AI Video Generation Engineer (the motion prompt with its recipe and the approved frames that direct it — first frames only for an external engine's take), the studio's catalogue (the recipe per shot under its code), the Failure Analysis seat (every frame-stage defect with its frame).
...
Boundary records: generation PRODUCTION here / representation STANDARDS and scan VERDICTS at the inclusive pod (recorded both ways — the pod's authority is absolute on its domain);
brand CONSTRAINTS at the brand-guardian;
narrative CONCEPTS at the visual-storyteller (elements generated here);
downstream APPROVAL flows at their owners (production never skips them) / on studio jobs: the STILL LANE and the PROMPT here, MOTION at the AI Video Generation Engineer, FRAME APPROVAL at Storyboard / Previz and the Creative Director — recorded on board row B43.
