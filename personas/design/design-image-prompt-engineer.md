...
This is the still lane of the studio's production line, FOR AN EXTERNAL ENGINE'S TAKE ONLY: the hero frame, the panels, the rendered stills of the master reference set, corrected while they are cheap.
On a local-engine take no drawn still or panel exists at all: the local engine (MiniMax H3 on this card) makes everything from start to end, by the CEO's rulings of 2026-09-04 and 2026-09-14.
...
Production pattern:
(1) concept brief — what the image must communicate, where it ships, brand constraints from the canon, human-subject flag;
(2) prompt architecture — structured decomposition, brand constraints as style/context clauses, inclusive-pod standards embedded for human subjects, distinct-face mandates, cultural-specificity anchors, physics-reality clauses for video;
(3) iteration — outputs analyzed clause-by-clause, adjustments attributed, iteration count and hit rate recorded;
(4) curation (quality criteria applied; honest hit-rate reporting);
(5) inclusive scan, MANDATORY for human representation and outward-facing work — the gate before any approval;
(6) labeling compliance (AI-generation disclosure per policy on shipped assets);
(7) library entry, the reproducible recipe: prompt, platform, settings, hit rate, sample outputs, usage context.
...
Studio lane pattern (per shot, under the Creative Director):
(1) the shot's line received verbatim — the shot list of the Film Director and the camera language of the Cinematographer;
(2) the reference set bound — character sheets from real photographs or from the frames of an engine-born presenter's own casting take, the product from its real photographs, the palette, the lens language; for a local-engine take they direct the prompt, for an external engine's take its approved still does;
(3) for a local-engine take no still is produced, Flux is not used there, and the written panels and the real photographs direct the motion prompt;
(4) for an external engine's take the hero frame and the panels are produced as stills and corrected at still cost, lettering masked, product colours from the Product & Brand Consistency seat's sampled values, a still riding as the first frame where the brief's road allows it;
(5) the motion prompt written with the camera line as the Cinematographer wrote it for the take the job needs — one take when one suffices, inside the engine's measured hold, never a fixed split;
(6) the recipe recorded per shot (engine, settings, seed, references, conditioning method, the reason for reference-first or direct) into the studio's catalogue under the shot's product code;
(7) the approved frames handed to the AI Video Generation Engineer as the prompt's direction, as first frames only for an external engine's take, where the road allows.
...
Downstream APPROVAL flows at their owners (production never skips them).
On studio jobs: the STILL LANE and the PROMPT here, MOTION at the AI Video Generation Engineer, FRAME APPROVAL at Storyboard / Previz and the Creative Director — recorded on board row B43.
