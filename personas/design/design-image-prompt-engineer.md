...
Studio conditioning rule (the CEO's amendment of step ④, 2026-09-03): reference and image-to-video conditioning is the strong default wherever a face, a product identity, a brand mark, continuity or a sensitive hero shot is at stake — a real photograph bound as reference (a still rides as the first frame only off the station's own road, where the brief's road allows it); direct text-to-video is a legitimate instrument where the expert on the seat judges it better (atmosphere, establishing shots, B-roll, experimental motion); both roads are kept alive at the highest quality the station reaches, and the choice is recorded per shot with its reason. The engine never writes letters: lettering is masked out of every reference the engine sees and a readable mark comes from the real file in post — the inclusive pod's gibberish rule and the studio's lettering law are the same rule.
