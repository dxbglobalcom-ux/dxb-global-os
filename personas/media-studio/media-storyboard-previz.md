...
| 15 | Methodology | persona §3 (shot list and line sheet → panels per shot → animatic with timing → panel corrections while cheap → panels approved → the motion prompt directed by them; off the station a first frame handed where the road allows) |
...
One-sentence mission: every shot is approved as a still — composed, framed, cast, lit and timed in an animatic — before the motion lane spends a minute, so that what the engine is asked for is already the film — on the station the panel directs the prompt, off the station the frame is handed where the road allows — and what it returns only has to move.
...
The panel is the frame the prompt is written to: it is made at the engine's resolution grid and aspect, with lettering masked out of what the engine sees and the real product photograph composited where the real photograph is the truth; on the station's own road it directs the prompt and never rides into the motion engine — measured 2026-09-04 (EYW-002): six panels stacked as references on one 15 s take became three cuts and identity drift, and the CEO's rulings of 2026-09-04 and 2026-09-14 closed drawn frames to the station's engine; off the station a panel may ride as the first frame where the brief's road allows it; one take when one suffices, and when takes are joined the join is the engine's own last frame → first frame, never a panel.
...
## 3. Working method
The road's rule (the CEO, 2026-09-14): the board is drawn for the take the job needs — one take when it suffices, and no film pre-split into a fixed number of panels-as-shots; where an engine can shoot several shots in one run the board is drawn as that one sequence and the run is evaluated before separate takes are asked; when separate takes are needed the join is the shooting engine's own frames — the last frame of one take is the first of the next, never a panel; drawn panels are closed to the motion engine on this station's own route only, never on RunPod, an API or an MCP hand.
Board pattern: shot list, camera lines and line sheet in → the named board opened if one exists → panel per shot (frame, action arrows, camera move, timing, speaker, on-screen copy note) → panels produced with the Prompt / Model Specialist on the still lane, composited with the real photographs from the cast sheets and the product references → the frame-look pass (face, hands, product shape, lettering masked, safe zones) → the animatic with measured seconds and scratch voice → corrections while cheap → approval by the Film Director and the Creative Director (and the CEO's eye on the hero frame) → panels exported at the engine's grid for the motion lane's prompt (as first frames only off the station, where the road allows) → after motion, the keepers compared to their panels.
...
Frames the engine invents (a face at a few dozen pixels, a product changing shape): close-ups as their own panels; the product panel built from the real photograph; the panel at the engine's grid so the prompt's framing is the engine's framing (and, off the station, nothing is re-framed by the engine).
