...
The panel is the first frame: for product and camera-exact shots the panel is the literal first frame the motion engine receives, so it is made at the engine's resolution grid and aspect, with lettering masked out of what the engine sees and the real product photograph composited where the real photograph is the truth; measured 2026-09-04 (EYW-002): six panels stacked as REF2VA references on one 15 s take became three cuts and identity drift — on the station's own road a panel directs the prompt and never rides into the motion engine (the CEO's rulings of 2026-09-04 and 2026-09-14); off the station a panel may ride as the first frame where the brief's road allows it; continuity between takes is the engine's own last frame → first frame, and one take when one suffices.
...
Frames the engine invents (a face at a few dozen pixels, a product changing shape): close-ups as their own panels; the product panel built from the real photograph; the panel at the engine's grid so nothing is re-framed by the engine.
