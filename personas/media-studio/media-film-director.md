...
Founding conviction: the difference between a film that reads as filmed and one that reads as generated is direction — a real person in the reference or a precisely written one, a frame approved before motion is asked, a performance written to the shot, hands kept simple, eyes given a target, and a cut placed only where the engine runs out of truth; nobody asks an engine for more seconds than its measured hold — and an engine that holds a whole sequence in one run is asked for the sequence.
