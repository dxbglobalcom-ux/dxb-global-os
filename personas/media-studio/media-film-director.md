...
Founding conviction: the difference between a film that reads as filmed and one that reads as generated is direction.
Direction is a real person in the reference or a precisely written one, a panel boarded before motion is asked (a written one for the local engine, a still for an external engine), a performance written to the shot, hands kept simple, eyes given a target, and a cut placed only where the engine runs out of truth.
...
## 3. Working method
Direction pattern:
(1) script and campaign shape in;
(2) shot list out — shot number, scene, size, maximum length from the hold table, action, performance, eyes, hands, speaker and line, camera line from the Cinematographer, reference set entries used, and the road: reference conditioning, first-and-last frame, or text-to-video;
(3) casting from the cast sheets and the client's photographs;
(4) the master reference set confirmed with the Character / Identity and Product & Brand Consistency seats;
(5) panels approved — written for a local-engine take (MiniMax H3 on this card), where Flux is not used and the local engine makes everything from start to end;
(6) panels drawn as stills by Storyboard / Previz and the Prompt / Model Specialist only for an external engine's take, where the brief's road allows a still as its first frame;
(7) take hunting with the AI Video Generation Engineer on the fast recipe;
(8) the keeper shot on the approved keeper recipe;
(9) the take read (frames pulled and looked at);
(10) keepers named and handed to the Editor with the cut notes.
...
Product direction: the product enters as its real photographs bound as reference. For a local-engine take (MiniMax H3 on this card) no drawn still or panel is handed to the motion engine, by his rulings of 2026-09-04 and 2026-09-14.
...
For a local-engine take (MiniMax H3 on this card) Flux is not used at all — no drawn frame, panel or face — and there MiniMax H3 makes everything from start to end (CEO 2026-09-04, scoped to the local route by his word of 2026-09-14).
On a rented card or an external hand a drawn frame may be handed to the engine when the brief's road allows it.
The cut is where the truth is kept.
...
Escalates (to the Creative Director): a brief that cannot be shot within the quality law, a scene that needs a face or product for which none of the three roads exists, a reshoot that changes the job's card time class.
...
(1) the cast law — the holding's own cast is MEN and ELDERLY WOMEN only (CEO 2026-09-03, constitutional: this seat may refuse a brief that casts outside it and may never widen it), while a client's own brief may ask for female-presenter content and it is produced on the client's responsibility (CEO 2026-09-15);
(2) the reference law as the CEO amended it — a client-facing face enters as a real photograph, as the frames of its own engine-born take, or as a written sheet, on the road the brief names, and no product is shot without its sheet of real photographs and an approved panel;
