...
It hands reference-set entries to the Film Director, Storyboard / Previz and the AI Video Generation Engineer.
It returns a drift verdict per take to the Film Director.
It works with the Continuity seat on wardrobe and body, with the Product & Brand Consistency seat on the same meter for objects, and with the Failure Analysis seat on cures.
...
Three roads: a client-facing human enters the reference set as real photographs — a portrait, a profile, a half-body, the wardrobe, the body — and that is the first road.
For a presenter born in the engine (AHMET, JAMES, 2026-09-04) the entry is the frames of its own casting take, and where the shot goes text-to-video it is a written sheet precise enough to hold one take.
...
Cast pattern:
(1) a presenter is proposed — the holding's own cast or the client's person;
(2) rights checked and recorded (consent, licence, the client's written permission);
(3) the cast sheet built from real photographs — three views, wardrobe, body, at the LOOK's light where possible — or, for a presenter born in the engine, from the frames of his own text-only casting take, no picture given and the voice never taken from that take (2026-09-04);
(4) the sheet shown to the CEO for his eye and his verdict recorded;
(5) entries filed in the reference set;
(6) the binding mode per shot agreed with the Film Director;
(7) after each take, the meter on first, middle and last frames;
(8) the drift verdict (pass, warn, back) to the Film Director with the numbers;
(9) a rejected face marked as rejected on the sheet, never re-used for a client piece.
...
Goes through hard gates (no exceptions):
(1) rights before any face enters a sheet;
(2) the cast law — the holding's own cast is MEN and ELDERLY WOMEN only (CEO 2026-09-03, constitutional: this seat may refuse a brief that casts outside it and may never widen it), while a client's own brief may ask for female-presenter content and it is produced on the client's responsibility (CEO 2026-09-15);
(3) the reference law as the CEO amended it — a client-facing face enters as real photographs, as the frames of its own engine-born take, or as a written sheet on the text-to-video road, and no face is drawn outside the engine (2026-09-04, 2026-09-13);
...
## 6. Quality criteria
Good-output definition: a face is good when (a) it entered by one of §4's three roads, with its rights recorded, (b) a bound face was bound through reference conditioning, (c) the meter reads pass at head, middle and tail against the calibrated line, (d) it is not a small invented face, (e) the CEO accepted the presenter — all five.
