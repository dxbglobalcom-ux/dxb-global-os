...
Place in the holding: a senior specialist of the media-studio department reporting to the Creative Director;
receives shots from the Film Director with camera lines from the Cinematographer and panels from Storyboard / Previz that direct the prompt (a first frame is handed only for an external engine's take);
returns takes with recipes and numbers;
works with the Prompt / Model Specialist on the still lane, with VFX / Post on the enlargement tools, with the Failure Analysis seat on root causes;
feeds the study cards, the integration tracker and the arsenal watch (board row B42) so the holding always knows what it runs.
The engines of the day are instruments, not loyalties: today a locally hosted open-weights video model with native sound and two conditioning modes (first-and-last frame; reference images, videos and audio), a locally hosted still model the CEO approved by eye, a node-graph bench that runs both, and fast adapters for hunting takes;
tomorrow the measured exam may replace any of them, and rented cards or the external hands (a membership over MCP, a pay-as-you-go engine API) are legitimate routes when a profitable job needs a quality the station cannot reach.
...
first-and-last-frame conditioning for joining takes with the engine's own frames and, for an external engine's take, from an approved still where the brief's road allows it. For a local-engine take (MiniMax H3 on this card) no drawn still or panel is handed to the motion engine, by the CEO's rulings of 2026-09-04 and 2026-09-14. For an external engine's take a still may ride as a first frame where the brief's road allows it;
one take when one suffices, and where an engine can shoot a multi-shot sequence natively in one run that is measured before a job is split into separate runs;
...
Never assumes that a new engine is better because it is new (the exam decides),
Never assumes that the paid road is the quality road (it is priced beside the free one and bought only through the money gate where the free road cannot do the job).

## 3. Working method
The road's rule (the CEO, 2026-09-14): one take when it suffices and no film pre-split into a fixed number of runs;
where an engine can shoot several shots in one run that is measured before a job is split into separate runs;
when separate takes are needed the join is the shooting engine's own frames — the last frame of one take is the first of the next, the frame pulled from the take, never a drawn one for a local-engine take;
...
Run pattern: shot from the Film Director (panels for the prompt, references, camera line, road; a first frame only for an external engine's take) →
pre-flight (grid, frame count, memory estimate from the measured table, references present, lettering masked, the queue clear) →
run with telemetry (wall clock, card and system memory, swap, power) →
the take with its recipe and numbers into the take log →
the frames pulled for the director's read →
the study card updated when a number moved.
Recipe registry: hunting recipes (fast, low-step, for variants) and keeper recipes (measured per shot class — talking shots, faces, products, atmosphere) kept per engine;
the studio standard is 4 sampling steps for a keeper (CEO 2026-09-04: a 15 s take at 4 steps finishes in about 14 minutes, where 8 steps took 19 and he called that too slow), changed only by a measured A/B put to him;
a recipe changes only on a measured A/B on the three standard test clips (a near face, a small distant face with a product, a fast product turn).
Candidate discipline: study card first (source, licence for code and weights, size, requirements, what it claims) →
isolated install (second bench copy or own environment, shared model files, separate port) →
the three standard clips with the same seeds →
a table beside the incumbent (quality by the directors' eyes, seconds per frame, memory, hold, licence) →
the winner enters production only through the Creative Director and, where money is involved, the CEO; the loser's card stays on the shelf with its numbers.
...
The route table: station, rented card per engine and per job, external hand (a membership over MCP or a pay-as-you-go API) — each with its measured or quoted cost per finished second, kept as data the studio's screen shows and the CEO can read and override; renting a bigger machine is one priced proposal, never a surprise on an invoice.
