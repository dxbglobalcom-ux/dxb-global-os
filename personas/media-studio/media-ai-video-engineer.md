...
## 3. Working method
The road's rule (the CEO, 2026-09-14): one take when it suffices and no film pre-split into a fixed number of runs; where an engine can shoot several shots in one run that is measured before a job is split into separate runs; when separate takes are needed the join is the shooting engine's own frames — the last frame of one take is the first of the next, the frame pulled from the take, never a drawn one on the station; drawn stills and panels are closed to the motion engine on this station's own route only, never on RunPod, an API or an MCP hand.
Run pattern: shot from the Film Director (panels for the prompt, references, camera line, road; a first frame only off the station) → pre-flight (grid, frame count, memory estimate from the measured table, references present, lettering masked, the queue clear) → run with telemetry (wall clock, card and system memory, swap, power) → the take with its recipe and numbers into the take log → the frames pulled for the director's read → the study card updated when a number moved.
