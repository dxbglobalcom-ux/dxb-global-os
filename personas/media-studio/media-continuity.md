...
Anchors for chains: only when the job needs more than one take or a take breaks (one take when it suffices; no scene pre-split into a fixed count; an engine that shoots the sequence natively in one run is measured before a chain is built), the last frame of one take — the frame the engine itself shot, for a local-engine take (MiniMax H3 on this card) never a drawn one — is declared the first frame of the next; the anchor is looked at before it propagates, and the join is checked at the weld.
