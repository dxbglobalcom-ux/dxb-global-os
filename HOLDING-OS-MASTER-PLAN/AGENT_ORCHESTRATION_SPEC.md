...
| # | Adaptation | Why |
|---|-----------|-----|
| A23 | **Code mode — an employee's intermediate results never enter its context.** <!-- OPEN: B41 --> Planned, not built. A chain of tool calls whose middle results the model never needs is run as one operation and only its answer returns. First candidate, measured 2026-09-19 with SELECT only: `tool_calls` holds 537 calls over 219 runs (2026-07-17 → 2026-09-05), `mcp__dxb-mcp__queue_get` 300 of them, `media_probe` 57, `memory_recall` 35 — 2.5 calls a run. Before anything is wrapped, the context cost of the `queue_get` chain is measured; a wrap that saves nothing measurable is not built. | The research fleet already works this way on the construction side (`fleet/fleet.sh`: scripts chain search → open → clean → count, the model sees the count). The employees' bus has no such operation, and the load today is small — so this is registered, not built. |
| A24 | **Ergonomic layer — a chain that recurs becomes one operation on the house bus.** <!-- OPEN: B41 --> Planned, not built. `packages/dxb-mcp` (9 groups: approval · audit · cost · crm · dashboard · media · memory · queue · registry) gains a wrapped operation only for a chain that the `tool_calls` record shows recurring; which chains recur is measured first, and the operation is named in `CAPABILITY_ARSENAL_DOCTRINE.md` §9 before it is written. | The film's claim 5; owned here because the bus is this spec's. No candidate beyond `queue_get` is measured yet, and none is invented. |
