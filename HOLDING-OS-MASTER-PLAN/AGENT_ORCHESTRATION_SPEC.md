...
`NoRouteError: no enabled routing_rules row matches task_class 'content.outbound'`. The row exists and is enabled at L1 — but its match condition is `{"keyword": "listeleme"}`, residue of the Outleteuro vertical slice (PHASE-05 §2, *"tek marka ürün listeleme taslağı"*) the CEO **cancelled** in U19. So the classifier is told `content.outbound` is a legal class, chooses it, and the router then refuses every outward-content task that is not a product listing. It is a data row and a routing decision, so it was put to the CEO rather than changed by the author. Row B40. <!-- OPEN: B40 -->
