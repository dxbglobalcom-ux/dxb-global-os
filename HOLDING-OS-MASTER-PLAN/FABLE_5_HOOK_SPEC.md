...
E10.2 boundary: spawn-path binding, `agents.hook_version` stamping, employee-card hook status field (§5 row 3), `run.finished.hook_result` payload (§9). Pre-gate <100 ms health-snapshot metric (§26) → P7 health row. **CLOSED 2026-07-14 — see the E10.2 adaptations below.**

## Registered adaptations — E10.2 (2026-07-14, Fable K1; ticket .planning/quick/20260714-e102-spawn-binding)

| # | Adaptation | Why |
|---|-----------|-----|
| A9 | Binding site: `runWorkerOnce` (packages/orchestrator worker-shim) — the ONE orchestrator dispatch that exists today (ORCHESTRATION §3 lifecycle). preTask runs BEFORE the observability runScope opens, so a block rejection means the run row is never created (§6 verbatim); post-gate + runtime monitors run INSIDE the scope (violations need the run id). The workflow executor (kernel, E9.1) keeps its own write-time gates (B7b approval-step wall, HR gate, budget) and adopts this same binding when the P7 resident worker unifies the two spawn paths — recorded boundary, not silent skip. | §3 fixes the binding point at orchestrator.dispatch; duplicating the binding into the workflow executor before P7 unification would create two divergent gate assemblies. |
| A10 | §6 evidence package source on the task path: EXECUTOR-DECLARED result keys (`evidence[]`, `acceptance_map`, `decisions_claimed`, `memory_written`) extracted mechanically. An executor that produced no evidence gets REVISE→ESCALATE — the binding never fabricates evidence (that is the exact failure std 7 exists to catch). Post-gate REVISE re-executes with the feedback riding `task.feedback` + std 11 purpose riding `hook_project_purpose` (certified Executor signature untouched — extras flow structurally). | §6's kanıt paketi needed a concrete carrier; the certified Phase-5 Executor signature is LOCKED. |
| A11 | §9 `run.finished` = the E8.3 envelope's terminal event names (`run.succeeded`/`run.failed`); their payload gains `hook_result`. The summary is persisted on `agent_runs.hook_result` (new jsonb column) by the run closure (§14 "gate kararları run kapanışında agent_runs'a") via `RunScope.setHookResult`. `agent_runs.hook_version` (new text column) carries the STARTING hook version (§27 verbatim). | §4 DDL predates the run-side columns; E8.3 envelope catalog has no literal `run.finished` event. |
| A12 | `hook.enabled` flipped to true in migration 20260714040000 (the E10.1 seed promise) + §21 backfill: all non-archived employees stamped v1 (archived rows stay NULL — honest history, employee card renders them neutral, not alarming). Live stamping on every passed pre-gate: `agents.hook_version` written when it moves (idempotent). Flag resolves per employee → global (`resolve_setting(key, agent)`); a failed flag read = ENABLED (fail-closed direction). | §21 acceptance + roadmap acceptance (spawn → hook_version dolu); §13 forbids per-employee opt-OUT, per-employee opt-IN scope stays for staged rollouts. |
| A13 | §22 "flag kapalı dönem alert'lidir" made mechanical: every spawn that flows past a disabled hook raises an `attention` alert (dedup `hook:disabled`, active-unique). Pre-hook certified suites (phase5/6 slices, E8 batteries — fixtures with no employee/persona/project surface) pin the flag off for their lifetime through one helper (`tests/helpers pinHookOff`) and restore+sweep in afterAll; production state is ON. | The flag is the spec's own gradual-transition mechanism; the pin is loud in the DB (alert row) — never a silent bypass. |
| A14 | Runtime monitors on the single-shot task path run post-execution (token budget vs run usage, confidence vs escalation threshold) — the §6 monitor/kill split holds (hook records, runner decides); mid-run streaming taps arrive with the P7 SDK session runner. | Today's certified executor is single-shot; there is no mid-run surface to tap yet. |

## Registered amendment — R1.5 (2026-07-17, Fable K1): `halal_screen` constitutional pre-gate check

Source: [[00-CEO-DIRECTIVE-REVENUE-FIRST]] §2 (immutable Islamic boundaries) + MASTER_PLAN §11 + REVENUE_ENGINE_SPEC §16 (Islamic gate triple, leg c). Measured baseline before this amendment: corpus+code grep islam|haram|halal|helal|alkol = 0 hits (2026-07-16).

- **Policy row `const.halal_screen`** (migration `20260717010000_r15_halal_screen_policy.sql`): gate `pre`, severity `block`, filed under standard_no 11 ("do not conflict with holding goals" — haram scope is the deepest conflict with holding goals; the 1..17 constraint stays untouched). The rule JSON carries the CANONICAL haram term list, 9 categories: alcohol, tobacco, pork, gambling, riba, adult, fraud, crypto_trading, stock_trading (crypto/stock trading = CEO exclusion, Talep §4).
- **Check semantics** (`packages/hook/src/pre-task.ts`): task text surface (objective + output_contract) is letter-boundary matched against the term list with tr-locale lowering on both sides; a hit fail-closes the spawn — REJECT + `hook_violations` row + `decision_log` `hook_reject` row (existing violation path). False-positive guards: no bare "ham"/"stock"/"slot"/"interest"/"bitcoin" terms; multi-word terms used instead.
- **Immutability:** policy edits remain CEO-ONLY through the `fn_hook_set_policy` wall (§13, `auth.uid()` required — the system actor is rejected). No agent can weaken, disable, or reinterpret the boundary; §27 CEO sovereignty applies unchanged (actor `ceo` degrades block → warn, recorded).
- **Settings mirror:** the R1.2/R1.3 revenue_core wave mirrors the canonical list into settings `revenue.halal_screen` for the opportunity intake screen (REVENUE_ENGINE_SPEC §16 leg b). Canonical source stays this policy row.
- **Evidence (executed 2026-07-17):** live policy row `const.halal_screen|11|pre|block|t`; `tests/e10/hook.test.ts` 35/35 PASS incl. 4 new cases — EN red (alcohol), TR red (kumar, tr-locale), letter-boundary PASS ("ham veri / stok raporu"), §27 CEO warn-degrade; spawn-binding 6/6 PASS.
