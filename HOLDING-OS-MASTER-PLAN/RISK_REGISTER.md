...
| R04 | K | Persona yazımı yetişmez (Fable-only, devredilemez) | O | Y | E5 erken paralel başlar (E4 sonrası hemen) → yetişmeyen "Fable-yazımı bekliyor" listesi; departman v2'siz AKTİVE EDİLMEZ | E5 sayım raporu | BACKUP_PLAN §1.6  <!-- HISTORY --> |
...
| R07 | A | RAM tavanı aşılır | O | D | Yeni-servis-yok kuralı (R5; motor/hook/router hepsi kütüphane) → STACK.md fallback (servis eleme sırası). **B39, 2026-08-25 — risk ölçüldü ve düştü, ama yok olmadı.** Ev artık 8 GB'lık kiralık kutu değil, **30 GB / 24 iş parçacığı** iş istasyonudur; eşzamanlılık ölçümünde **sekiz hattın toplam bellek maliyeti 4,5 MB** çıktı, yani hat çoğaltmak bu tavanı zorlamıyor. Risk **kiralık cx33 kutusu için duruyor** (hâlâ 8 GB) ve asıl RAM tehlikesi bu satırın hiç öngörmediği yerden geldi: 2026-08-17 akşamı bir oturumun yazdığı arama kalıbı 380 baytlık girdi için **26 GB**'lık bir eşleştirici kurdu ve makineyi dondurdu. Tavanı aşan şey servis sayısı değil, tek bir komut oldu. | System Health RAM eşiği %85 · `pnpm bench:drain` bellek sütunu | SYS_ARCH R5 |
