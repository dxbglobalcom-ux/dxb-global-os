...
| # | Opened | What is open (plain language) | Owning spec / ledger | Why it is still open | Waits on | What closes it |
|---|---|---|---|---|---|---|
| B21 | 2026-07-30 | **The context architecture** — one always-on core instead of 27 pages, eight doors that load only for the job at hand, a state file that is a photograph instead of a novel, machine gates against re-duplication and against false approval claims, dead rules deleted, and Hamza's information package put through the same discipline | `.claude/CLAUDE.md` · `.claude/skills/dxb-*` · [[00-CEO-DIRECTIVE-MEASURE-NEVER-GUESS]] · `scripts/governance/{rules,ceo-approvals}.json` | Approved and built the same night; the CEO's own eye has not yet accepted it (LAW B) | CEO (his reading) | He reads it and says the sessions now start clean — or names what is still wrong |
