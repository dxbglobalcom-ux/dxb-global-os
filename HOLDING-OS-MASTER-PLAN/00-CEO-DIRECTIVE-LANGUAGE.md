...
3. The rule binds ALL authors: construction sessions (the author is the model that runs the session), GPT 5.6 solo module work (K1), future hr-factory output, and every runtime agent producing repo/DB artifacts. *(Historical: written while Fable 5 held authorship.)*
