...
- **A4 compiler semantics:** subject's granted set = PERMISSIVE UNION (employee ∪ department ∪ role_level, §19); applied as INTERSECTION onto the policy profile (PERMISSION_MODEL G2 — library can narrow, never widen). Subject PRESENT in the layer = record governs (empty set = deny-all); subject with ZERO grants = registry behavior unchanged (§22 backward compatibility). Employees holding employee-kind grants get `<slug>.employee.mcp.json` overlay profiles (= dept surface ∩ their union). **W9 adaptation (CEO 2026-09-15 "onay" <!-- CEO-OK: w9-assigned-seats-plan-approved-2026-09-15 -->):** an employee ASSIGNED to another department (`agent_assignments` — a second membership, never a move) has that department's surface added to its CEILING, so the overlay = (home surface ∪ every assigned surface) ∩ its union. The cut stays an INTERSECTION and G2 is untouched: the employee still cannot exceed the surface of a department it is a recorded member of — what widened is the MEMBERSHIP in the org record, not the library's power over the policy. The assigned department's own GRANTS are deliberately not merged into the union (least privilege: a borrowed seat adds only what it holds by name, never every tool the host department may gain later). Measured on the two studio seats 2026-09-15: home kit 23 → overlay 28, the five `media_*` tools and nothing else; the home department profiles still emit 0.
