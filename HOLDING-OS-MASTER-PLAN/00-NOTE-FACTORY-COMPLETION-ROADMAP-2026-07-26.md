...
|---|---|---|
| 3.1 | Prove Hermes is alive on `dxb-vps-1` (SSH was refused this session — likely fail2ban after a failed key attempt; that is the beginning of the work, not the end) | a live status read, not a memory of a deployment |
| 3.2 | Surface Hermes inside the Holding: visible, controllable, and its jobs legible to the CEO | a dashboard surface showing its state and last run |
