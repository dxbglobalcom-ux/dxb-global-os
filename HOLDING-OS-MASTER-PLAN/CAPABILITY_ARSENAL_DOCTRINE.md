...
| **Apify Instagram Scraper + Instagram Reel Scraper** (Apify actors, via Apify MCP) | usage-based, ~$1.50 / 1,000 results; Apify MCP server itself is free to run, only actor usage is billed | deep Instagram extraction for research — profile/post/Reel/hashtag/comment pulls, and per-Reel caption + transcript + view/like/comment counts that a plain web read cannot reach | **a research task needs Instagram video (Reel) transcript or engagement counts that Instaloader cannot reach, or Instaloader is rate-limited/blocked** | **Instaloader** (free, open-source, self-hosted, ~13k★) covers profile/post/Reel/story/hashtag/comment/metadata without a paid actor; `instagrapi` (~6.8k★, private-API access) sits behind it as a controlled fallback for account-authenticated pulls only, never as the default — its own docs warn of account-trust and rate-limit risk in production |
...
The Instagram row was studied 2026-09-22 from an outside comparison (Apify vs Apify Reel Scraper vs Instaloader vs instagrapi vs Bright Data vs gallery-dl vs the official Graph API) run before the holding earns revenue: **Bright Data** was compared and set aside as enterprise-heavy for this stage, not entered on the bench; **gallery-dl** (19.8k★, the largest count of the field) was compared and set aside on the same measured principle as the rest of this bench — star count is not production reliability, and its Instagram path carries open breakage reports as of Sep 2026. Free-first stands: Instaloader is the default hand until a real task proves it insufficient, exactly as scrapling+camoufox stands ahead of Apify's own general-scraping row above.
...
**Registered 2026-09-19, on his word (B41; `opening-discipline-ordered-for-construction-and-holding-2026-09-19`) — two needs, measured before any tool is added:** <!-- OPEN: B41 --> (1) an **ergonomic layer** — a chain the `tool_calls` record shows recurring becomes ONE operation on this bus, named here first; the only chain measured so far is `queue_get` (300 of 537 calls, 219 runs, 2026-07-17 → 2026-09-05), and no operation is written until its context cost is measured (AGENT_ORCHESTRATION_SPEC A23–A24); (2) **code mode** — the wrapped operation returns its answer and never its intermediate results. The construction side already has both in the research fleet; the holding side gets them when row B41 reaches them, not before.
