...
function ledgerIdentity(): LedgerIdentity | null {
  try {
    ...
    const c = f.company;
    if (!c?.sysid || !c?.dboid || !c?.dbname) return null;
    return f;
  } catch {
    return null;
  }
...
/**
 * Anything that can run a query — the pooled handle, or a transaction. A kysely
 * `Transaction<DB>` extends `Kysely<DB>`, so one type serves both, and that is
 * the point: the check below is run on the very connection that writes.
 */
type Db = ReturnType<(typeof import("@dxb/shared"))["getDb"]>;
 ...
 */
async function whoIsThis(run: Db): Promise<(Identity & { datmap: string[] }) | undefined> {
  const { sql } = await import("kysely");
  const r = await sql<Identity & { datmap: string[] }>`
...
function refusalFor(here: (Identity & { datmap: string[] }) | undefined, ledger: LedgerIdentity): string | null {
  if (!here?.sysid)
  ...
  if (isSame(here, ledger.company))
    return (
      `DXB_CONSTRUCTION_DATABASE_URL reaches the company database ` +
  ...
  if (c && here.sysid === c.sysid && namedHere && namedHere.slice(0, namedHere.indexOf(":")) !== c.dboid)
    return (
      `the recorded identity of the company is stale — a database named '${c.dbname}' exists on ` +
  ...
  if (!ledger.allowed.some((a) => isSame(here, a)))
    return (
      `${here.dbname} (cluster ${here.sysid}, oid ${here.dboid}) is not a permitted ` +
