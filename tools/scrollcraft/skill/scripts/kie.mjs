...
// ---------------------------------------------------------------- key ----
// DXB ADAPTATION (2026-08-27, CAPABILITY_ARSENAL_DOCTRINE INTEG-01 triage, finding PE3):
// upstream walked up to 8 parent directories reading every `.env` it met. Inside this holding
// that walk can reach company secrets, which the secrets boundary forbids. The search is now the
// process environment first, then a `.env` in the current directory ONLY. No upward walk.
function findEnv(start) {
  const p = path.join(path.resolve(start), ".env");
  return fs.existsSync(p) ? p : null;
}
function loadKey() {
  if (process.env.KIE_AI_API_KEY) return process.env.KIE_AI_API_KEY;
  const envPath = findEnv(process.cwd());
  if (!envPath) throw new Error("KIE_AI_API_KEY not set and no .env in " + process.cwd());
