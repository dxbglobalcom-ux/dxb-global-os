...
| **07** | Misael · Founder Systems (`misael.systems`) | `media/07-DbSGx3CCRQS.mp4` | `56d2203b3333185e…` | `frames/07/` — **82 frames** + **4 native zooms** (`zoom/comms-16.0`, `zoom/comms-19.0`, `zoom/math-78.2`, `zoom/math-79.0`) | `transcripts/07.json` — en, 18 segments | **`07-misael-founder-systems.md` — written 2026-08-08 from nothing.** Watched 00:00→01:22 with sound. The two `comms` zooms are what made the inter-agent message grammar quotable; the two `math` zooms came back motion-blurred and the price table was read off native frames `t076`/`t079` instead |
| **08** | `paperclipai/paperclip` — *"the open-source app everyone uses to manage agents at work"* | `repos/paperclip` — 112M, HEAD `7797995` (2026-07-27) | — | — | — | **`08-paperclip-repo.md` — written 2026-08-08 from nothing.** 1,035,865 lines of TS/TSX, 197 SQL migrations, **109 schema tables read one by one**. Liveness measured against the GitHub API the same session: **75,835 stars · 14,120 forks · 5,080 open issues · last push 2026-08-07 22:25 UTC** |
| **09** | Huw Prosser (`huwprosser`) | `media/09-DYK2IWyoEWh.mp4` | `26a91b04f9e9e4ea…` | `frames/09/` — **15 frames** + 6 scene cuts + **2 native zooms** (`zoom/statusbar-11.0`, `zoom/dock-9.0`) | `transcripts/09.json` — en, 3 segments; **one meaning-changing error corrected from the screen** — the STT hears "Travis", the screen says `JARVIS` | **`09-huwprosser.md` — written 2026-08-08 from nothing.** All 15 seconds, every frame in order with sound. The dock zoom is what made the four control chips quotable: `[reasoning:off] [voice:on] [new chat] [sleep]` |
| **10** | `cloud9.markets` — "Nimbus, multi agent trading desk" | `media/10-DbBAOdVBjka.mp4` | `36bf05ac45b79902…` | `frames/10/` — **75 frames** + **2 native zooms** (`zoom/nimbus-role-12.0`, `zoom/localvoice-62.0`) | `transcripts/10.json` — en, 15 segments | **`10-cloud9-markets.md` — written 2026-08-08 from nothing.** Watched 00:00→01:15 with sound. All ten node labels and three group labels transcribed; the role-line zoom is what made `MASTER · STRATEGY SYNTHESIS / RISK ROUTER` quotable. **The trading business is explicitly NOT taken — only the way nine agents introduce themselves** |
| **11** | Rinaldo Janjua (`rinaldojanjua.ai`) — the five-stage Inbox Cleanup diagram | `media/11-DbA3JgbphEs.mp4` · **720×1280 — the lowest-quality source watched, meets his floor exactly** | `c76361d6bee7dad5…` | `frames/11/` — **97 frames** + 1 scene cut; **no zooms needed**, card text legible natively | `transcripts/11.json` — en, 25 segments | **`11-rinaldojanjua-a.md` — written 2026-08-08 from nothing.** Watched 00:00→01:37 with sound. All five stage cards, their badges, file paths, chips, the `CONSTRAINT mailto List-Unsubscribe only` object, the `AGENT BRAIN` globe and the `RUN BY COO` pill transcribed as text |
