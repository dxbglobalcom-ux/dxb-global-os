...
|---|---|---|---|
| **P1** | **Turn the voice supply back on and prove it.** `dxb_speaches_local` has been `Exited (0)` for two days; Kokoro cannot answer while the container is down. Restart it, then attack the two measured defects: **41.2 % empty transcripts and a 41.0 s round trip** | **W-C42-1** — already worded as *"kill the 44 % `empty_transcript` and the 29–35 s round trip"*. **Both figures are now re-measured and worse than the board says: 41.2 % and 41.0 s.** The row is corrected, not duplicated | AUTHOR |
