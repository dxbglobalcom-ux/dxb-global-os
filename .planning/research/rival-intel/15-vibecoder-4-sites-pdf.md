...
| **P15-4** | **The lead-magnet press** (`.md` → A4 PDF) | Rebuild §3.5's pipeline with what is already installed: Playwright's `chromium-1228` renders a Markdown document to a branded A4 PDF with our own five-colour system. It gives the 49 written marketing and social employees an artefact they can actually produce, and it is the missing manufacturing step under the clipping-business project of source 05 | **none** — Playwright + Chromium already on disk | **€0** | none to build; publishing anything outward stays on the gate |
