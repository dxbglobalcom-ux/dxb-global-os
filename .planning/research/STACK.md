...
**Confidence:** MEDIUM (versions verified against npm/PyPI registries on research date; pattern recommendations cross-checked across multiple independent sources; per classify-confidence seam, cross-verified web findings cap at MEDIUM)

**Constraint note:** This research treats the locked decisions in the approved master plan (§9, §10) as fixed constraints, not open questions: Supabase/Postgres for state+CRM, Hetzner-class EU VPS, OpenRouter primary + NVIDIA Build dev tier, hermes-agent as 24/7 resident harness, Obsidian + Graphify + open-notebook + claude-mem memory stack, custom CRM (no external CRM), in-house council (no llm-council dep). This document fills the remaining stack gaps around those constraints.

---

## Headline Recommendation

**One-language OS core: TypeScript/Node everywhere you write code; Python only inside containers you don't modify.** The Claude Agent SDK, the MCP SDK, Next.js, pg-boss, and supabase-js are all first-class TypeScript. Making the 8 DXB MCPs, the queue workers, the kernel, and the dashboard one pnpm monorepo eliminates a whole class of integration friction. LiteLLM, Speaches (voice), and open-notebook are Python — but they run as Docker services you configure, not code you write.
...
| **edge-tts** — ⛔ **CANCELLED FOR VIDEO, 2026-09-15** | was `~/.local/bin/edge-tts` (binary still installed; nothing outside the repo was removed) | none — do not reach for it | The Media Studio's `voice` job kind used it to lay a TTS track under a take. The CEO cancelled TTS for **every** video production on 2026-09-04 (*"şu yapay sesi iptal et tüm video üretimlerinde reklamdan tut filme kadar. MiniMax H3'ün kendi sesi olsun"*) and the hand went on offering it for eleven days. Removed on his word of 2026-09-15 — *"kaldır"* <!-- CEO-OK: w7-voice-hand-removed-2026-09-15 -->: the kind left `media_jobs`'s CHECK on both engines, `media_submit` and the lane's runner. **A film's voice is the take's own** — the spoken lines ride in the `shoot` prompt. ⚠ **This row is about VIDEO only.** The holding's own speaking voice is the Speaches row above (Hamza: `speaches:piper`), which shares no table, no binary and no line of code with the media lane and is untouched. |
...
```
CEO (laptop/phone) ──► Next.js dashboard (Vercel-style app served from VPS, Supabase Auth)
        │ voice: voicebox client ◄──► Speaches (STT/TTS, VPS)
        ▼
Kernel/Orchestrator = Claude Agent SDK programs (Node, VPS + laptop sessions)
        │ decompose → INSERT INTO pg-boss queue (same Postgres tx as state write)
        ▼
pg-boss workers (Node) ──spawn──► Agent SDK subagents / claude -p / Codex CLI / hermes
        │ every LLM call (API-tier) ──► LiteLLM proxy ──► OpenRouter / NVIDIA Build
        │ every tool call ──► DXB MCP Gateway (per-department tool allowlist)
        ▼
Supabase Postgres: tasks, CRM, approvals, audit, cost ledger, pgvector
        │ DB triggers → Realtime Broadcast → dashboard live updates
        ▼
Approval Gate MCP holds outward actions as DRAFT rows until CEO approves in dashboard
```

**MCP Gateway pattern (for P4):** the industry-standard design is a single enforcement proxy in front of all upstream MCP servers: clients connect to per-server proxy URLs, and the gateway (a) filters `tools/list` responses per agent identity **before discovery** — the model can only pick tools it sees — and (b) validates every invocation against that identity's default-deny allowlist, with gated tools (Stripe, DocuSign, email) diverted to the approval queue. Subagents get their own minimally-scoped credentials and never inherit the orchestrator's permission set. Implement the gateway itself in TS on `@modelcontextprotocol/sdk` (Streamable HTTP), with the agent-ID→allowlist mapping living in the Agent Registry tables. Study `docker/mcp-gateway` and IBM ContextForge as pattern references before building (study-card rule), but expect to own this code — per-department scoping tied to your own registry is the product.

**Dashboard realtime:** use **Supabase Realtime Broadcast (broadcast-from-database triggers)**, not `postgres_changes`, as the default. `postgres_changes` runs through logical replication (~50–200ms, per-event RLS checks, degrades past ~1k changes/s); Supabase's own current guidance recommends Broadcast for most use cases. At CEO-dashboard volumes either works, but Broadcast is the future-proof default and costs nothing extra.

**Memory/RAG layer (P3, wiring the locked components):** Obsidian vault = human-legible source of truth on disk (git-versioned); Graphify = knowledge graph over it; open-notebook = research brain service in compose (replaces NotebookLM before Oct 2026); claude-mem = session-memory hooks. The **Memory Router MCP** is the custom piece: a TS MCP that routes writes (decision → vault + graph; artifact → vault; research → open-notebook) and serves reads via pgvector semantic search over embedded vault/notebook chunks. Use Supabase's automatic-embeddings pattern (queue + trigger) so embedding stays inside the existing queue machinery. Embedding model via LiteLLM (cheap OpenRouter embedding model) to keep even embedding spend in the ledger.

## Installation

```bash
# Monorepo core (Node 22 LTS, pnpm)
pnpm add @anthropic-ai/claude-agent-sdk @modelcontextprotocol/sdk pg-boss @supabase/supabase-js @supabase/ssr zod
pnpm add next react react-dom ai            # dashboard package
pnpm add -D typescript supabase tailwindcss
...
**If the VPS is 8GB (CX42/CPX31-class, ~€16–20/mo):**
...
**If a department needs a Python-native capability (research stack, open-notebook):**
- Wrap it in FastMCP 3.4.2 behind the gateway rather than porting; the gateway makes language invisible to agents.

**If subscription-tier calls (Claude Code, Codex CLI) need cost visibility:**
...
---
*Stack research for: DXB Global OS — AI-native company operating system*
*Researched: 2026-07-05*
