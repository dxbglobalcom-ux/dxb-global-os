...
- **Open with him — asked, not answered:**
  ...
  4. ~~trade licence~~ — **dropped by his word** (typed into the Fable study session, 2026-09-24 ~19:55): *"legalliği şuan için boşver biz müşterilere yurt dışında hizmet veriorz boşuna resmi kaynakları araştırmada vakit token harcama"*. Do not raise it.
...
**A first version landed** (`363dcbab`, 19:43 — `STUDY.md` 5,869 words, `EVIDENCE.md`, the Fable session's own `HANDOVER.md` / `WORKLOG.md`; it handed over at 84 % context, and he set a 37 % rule for that job). **The study is still being extended** by a Fable 5.1 successor (VS Code tab "dxb ceosu solfaki fable araştırması", seen at 19:55 pricing agencies country by country) and he is driving it directly there. Do not audit a moving file: watch the folder's git log and the Fable session's `WORKLOG.md` for its own "done", then check the final version.
