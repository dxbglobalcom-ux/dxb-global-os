...
| Lab loose ends | `git status --short` in Medya OS · `ls -la tools/h3/studio/media` | ` M 01-hat/kapi.py` · `?? 01-hat/deliver.sh` · `dxb-v-oe-006.mp4 -> lab/out/DXB-V-OE-006.mp4` (un-gated) |
| Showcase catalogue lacks OE-005/006 | `grep -o 'DXB-[VGAS]-…' studio/index.html \| sort -u` | OE-001…004, BDR-001, LAB-001/002, G-LAB-001, G-OE-001 — no OE-005, no OE-006 |
...
| 9/9 redo shots failed the speech gate; OE-005 rejected by CEO | `Medya OS/99-log/GUNLUK.md` block "22:38 → 00:10" | `9/9 failed kapi ses (WER 0.11–2.4 vs 0.10), 3/9 failed kapi kare` · *"berbat yaşlı kadın… ayakkabı bozuk"* |
