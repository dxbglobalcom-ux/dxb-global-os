...
if [[ $part == baseline || $part == B ]]; then
  ...
  if [[ $part == B ]]; then
    ...
    chk "B5b the two overlays carry the five media_* tools (5+5)" "$(grep -c '"media_' "$D"/packages/gateway/profiles/design-image-prompt-engineer.employee.mcp.json "$D"/packages/gateway/profiles/marketing-short-video-editing-coach.employee.mcp.json 2>/dev/null | awk -F: '{s+=$2} END{print s+0}')" "^10$"   # added 2026-09-15 by the checker: B4 saw the pointer, nobody measured the file it points at
    chk "B5c resident scheduler started AFTER the gateway build (old code rewrites the overlays)" "$(( $(date -d "$(systemctl --user show dxb-scheduler.service -p ActiveEnterTimestamp --value)" +%s) - $(stat -c %Y "$D"/packages/gateway/dist/library-profiles.js) ))" "^[0-9]+$"   # negative = the running process predates the build it should carry
