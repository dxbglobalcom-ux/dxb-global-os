...
**5. WHAT WAITS ON HIM:** his click on the next topic — the call-sheet dispatcher (plan → one task per named seat with `depends_on`) and then the studio's own production of the sunglasses UGC and the OE-001 remake (the hands are built and proven, B43) · the A/B's 2K verdict when it lands · everything under "What is open" below (unchanged). <!-- OPEN: B43 -->
