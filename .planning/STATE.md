...
**2026-09-03 — THE MEDIA STUDIO IS FOUNDED AS A DEPARTMENT, TEN OF ITS SIXTEEN EXPERTS ARE WRITTEN AND GATED, AND THE SESSION WAS CLOSED BY THE CEO BECAUSE IT GREW TOO LARGE. THIS IS THE CURRENT POSITION; the 2026-09-01 block below is the previous session's, already answered.** <!-- OPEN: B43 -->
...
- **10 of 14 personas written in person, tool-agnostic experts** (`personas/media-studio/`): media-creative-director · media-advertising-director · media-film-director · media-cinematographer · media-screenwriter · media-storyboard-previz · media-ai-video-engineer · media-character-identity · media-product-brand-consistency · media-continuity — each: mechanical gate PASS (`packages/hr/dist/gate.js`), `scripts/sync-personas-to-db.sh` with `DXB_PERSONA_AUTHOR=fable-5`, `fn_persona_gate(...,'passed',...)`, `agents.persona_id` bound, `persona_version='v2.0-fable'`, `--verify` parity PASS; `tests/r31/persona-delivery.test.ts` 4/4.
- The four defects' root causes confirmed on frames: the shoe morph exists only in `DXB-V-OE-001` (one 15 s take) and not in `DXB-V-OE-002` (3–4 s panels) — the long single take is the cause, as he observed.
...
**1. FINISH THE SIXTEEN — four personas + two upgrades, same pattern, same rigour.** Employee IDs already exist in the company (`select slug,id from agents where department='media-studio'`): `media-vfx-post` `0afc6fb1-06fb-4380-b675-ca157cf3b8ae` · `media-sound-music` `298d1ce4-2428-456b-8d8c-70dde2d74339` · `media-delivery-qc` `63130ba5-fadd-4002-9516-d82c4faab747` · `media-failure-analysis` `4c1b9e38-1717-434b-9518-0ed0ccff0982`. Each file: the 33-field dossier (copy the Creative Director's shape, role-specific values, field 8 never names a model), the `# PERSONA — <Title>` header, §1–§11 role-specific (≥3 content lines each, no `{placeholder}`, no model name, tool-agnostic — "the engines of the day", other local models allowed), §12 byte-identical to any existing persona's §12. Then per file: `node` one-off calling `gatePersona(body)` from `packages/hr/dist/gate.js` (result `{ok, failures}`) → `DXB_PERSONA_AUTHOR=fable-5 scripts/sync-personas-to-db.sh <file>` → `select fn_persona_gate('<persona id>','passed','<your review note>')` through `docker exec -i supabase_db_DxB_Global_OS psql -U postgres -d postgres` → `update agents set persona_id=…, persona_version='v2.0-fable'` → `scripts/sync-personas-to-db.sh --verify <file>` must print `VERIFY: PASS` → `npx vitest run tests/r31/persona-delivery.test.ts`. Content owed to each: **VFX / Post** — enlargement (the SeedVR2 A/B is its first measurement), grade, grain, halation, overlays and real-photograph inserts from the Product seat, "enhancement is the last layer, never make-up"; **Sound / Music** — one voice source per shot (engine track alone if it matches the script within a measured word-error line, else muted and replaced; never mixed under a line), the audio head fragment trim, music licensing, loudness, the spoken-line check against the Screenwriter's line sheet; **Final Delivery / QC** — the light last door (three numbers only: identity, language + spoken words, lettering), the product code `DXB-<TYPE>-<CLIENT>-<SEQ>`, the catalogue and the showcase card, the deliverable matrix as the format checklist, never accepting on the CEO's behalf; **Failure Analysis / Optimization** — root cause at the step, the cure installed there, the error registry that closes a defect only with a rule, "no QA bureaucracy". **Upgrades (in place, `git mv` nothing):** `personas/design/design-image-prompt-engineer.md` gains the Prompt / Model Specialist seat of the studio (video engines and the still lane of B43, tool-agnostic, the studio's camera line from the Cinematographer) and `personas/marketing/marketing-short-video-editing-coach.md` gains the Editor seat (six to ten short shots cut into the piece, the continuity sheet as the cut's checklist, the assembly cut reviewed against the brief); both re-synced and re-gated as a new version. Then show him the department in his language (16 names, what each does in one line) and ask with ONE click-question whether the department may go to activation.

**2. THE UPSCALER — MEASURE, THEN SHOW HIM.** He ordered a Topaz-equivalent that is local, free, Linux, ONE tool, no cloud, no paid product — and *"1 videoyu yüz yılda üretmeyeceğiz"*. Research verdict: SeedVR2 via the nodes already inside `/home/dxb/tools/ComfyUI` 0.34.0. Nothing is measured on this card. Ask him with one click-question for the GPU minutes, then: a second bench copy (never the production venv `/home/dxb/tools/ComfyUI/.venv`; shared model files via `extra_model_paths.yaml`; port 8189), weights `seedvr2_3b_int8_convrot` (3.46 GB) and the VAE, the three standard clips at 1080p and 4K, seconds per frame, peak VRAM, a frame ladder at 100 % zoom, and the report to him with minutes per 15 s clip. Never propose Astra or any paid product.

**3. EXPO — HIS HAND, YOUR MEASUREMENT.** He chose "after the research"; the research is done. Procedure and after-run command: project memory `expo-after-measurement-2026-09-03.md` and the desktop file `GOREV-CUDA-EXPO.md` (BIOS → Ai Tweaker → EXPO I → F10; the reboot kills every session; after boot: dmidecode 6000 MT/s, then the 864×480 · 15 s · 4-step · seed 42 run beside 295.1 s; write the figure on B42). Ask him with one click-question when.

**4. LAW A LEFTOVER (small, do it when the shooting scripts are next touched):** the three prompt lines `tools/h3/oe/SCRIPT.md:30`, `tools/h3/oe/stills.sh:10`, `tools/h3/oe/fix-stills.sh:9` still ban third-party brand marks; his ruling deletes the ban (lettering masking stays).

**5. WHAT WAITS ON HIM:** his eye on the department (LAW B) · the GPU minutes for the SeedVR2 A/B · the EXPO hour · activation of the sixteen · everything under "What is open" below (unchanged).
