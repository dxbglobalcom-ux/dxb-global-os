...
**2026-09-04 — THE PICTURE-FREE TURN, AND THE DEPARTMENT'S FIRST EXAM.** <!-- OPEN: B43 --> His day, measured: EYW-001 v2 accepted 01:12 (3 photographs, engine voice). EYW-002 (his Flux-storyboard experiment, 6 panels bound) → three cuts + face drift, failed the studio. 17:05 *"hepsini MiniMax'e yaptır ama resim verme"* → EYW-002C, words only, one take, WER 0.033 → **accepted 18:45** (640p and its SeedVR2 1080p both, *"ikisi de güzel"*). AHMET (DXB-A-010) born inside the engine from a text-only casting take, named by him 17:14. 20:50 *"öyle bir kural olmamalı"* → **LAW E**: no picture to the motion engine; the real-photograph reference law replaced in seven media-studio personas (v3, synced, gated, commit 5ad40315). 19:00 *"1080 gerek yok"*: no upscale unless he asks. Engine standard 4 steps (15 s ≈ 14 min; 8 steps = 19 min is too slow for him). **The department's first exam (he as Hamza, 20:12–20:52):** his own 594-word prompt, one line changed on his order to four spoken lines, handed to the Creative Director; nine seats ran it through the station's hands; DXB-V-EYW-003 on the vitrin; ses WER 0.000, OCR 0; **the studio's own verdict: fails** (glasses float and change shape 4.4–6.9 s, box never opens, a model's face) — waiting for his eye. Station facts learned: the Bash tool's scope has memory.max 16 GiB (heavy jobs run as `systemd-run --user`); a 121-frame SeedVR2 segment needs 27 GB RSS → 60-frame segments; `start-server.sh --reserve-vram 3.5`. Lab record: `Medya OS/99-log/GUNLUK.md`.
