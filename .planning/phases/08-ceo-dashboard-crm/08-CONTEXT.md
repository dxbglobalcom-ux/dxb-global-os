...
- **"3 boyutlu"nun teknik karşılığı:** CSS derinlik dili — Double-Bezel nested yüzeyler (soft-skill), katmanlı ışık/gölge (bg-hue-tinted), cam yüzeyler SADECE sabit chrome'da (nav/overlay; scroll içeriğinde blur yasak), spring-physics motion (stiffness~100 damping~20), hover'da mikro-parallax. **Three.js/WebGL İZİNLİ — UI-SPEC Amendment A1.1 (2026-07-10) ile CEO kararıyla yasak kalktı:** render istemci GPU'sunda (hedef donanım RTX 4090 Linux laptop), VPS RAM'ini bağlamaz; imza anlarıyla sınırlı + CSS derinlik diline graceful fallback (bkz. 08-UI-SPEC.md §10).
