...
| 01-04-T3 | 01-04 | 3 | SEC-01 | T-01-14, T-01-15 | zero unresolved rows via DEAD?-column parse (review HIGH-2); ✅ rows timestamp + non-empty Observed (MED-4); ATT table complete (HIGH-3); grace literal column-parsed (MED-2); value-free | integration | column-aware awk on DEAD? column prints 21-rows 0-unresolved; MED-4 awk prints 0; ATT awk prints ATT-OK (ATT-01 = YES); CRED-17 Action-column awk prints GRACE-OK; `gitleaks dir <evidence dir> --redact` exits 0 | ✅ (template from 01-03) | ⬜ pending |
...
| 01-06-T2 | 01-06 | 5 | SEC-01..04 | T-01-23, T-01-26 | every hard-gate criterion re-run live and passing; SEC-01 via column-aware parsers incl. Observed + grace column (review HIGH-5, MED-4, MED-2); A8 blocking rule parsed from evidence/a8-deny-check.md (review HIGH-1) | integration | gate report exists; red-cross count = 0; report shows 21-rows 0-unresolved + MED-4 check 0 + ATT-OK + GRACE-OK; exactly one A8-RESULT line parsed; live gitleaks re-run exits 0 | ❌ W4 | ⬜ pending |
...
- Apify grace-window trap: old token stays valid 24h if grace accepted — checklist must decline grace or delay probe. CRED-17's Action cell must carry the structured literal `grace=declined` or `grace=accepted; probe_after_24h=yes`, replacing the template's `grace=__PENDING__` placeholder (column-parsed and machine-gated in 01-04-T3 and the 01-06 gate report, review MED-2).
