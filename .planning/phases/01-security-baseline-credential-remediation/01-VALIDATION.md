...
| 01-02-T2 | 01-02 | 2 | SEC-02 | T-01-06 | agent tooling denied read of vault; A8 failure triggers out-of-workspace-vault compensating control (review HIGH-1) | smoke | settings.json parses; deny array contains the three Read entries; A8 outcome recorded — FAILED requires 'OUT-OF-WORKSPACE VAULT' block in .env.example (blocking-gated in 01-06) | ❌ W0 | ⬜ pending |
| 01-03-T1 | 01-03 | 2 | SEC-01 | T-01-09, T-01-10 | 21 pre-assigned evidence rows + 5 machine-parseable ATT attestation rows; tick-without-probe structurally impossible | smoke | `grep -cE '^\| CRED-[0-9]+'` = 21; `grep -cE '^\| ATT-0[1-5] '` = 5; key-shape grep = 0 | ❌ W0 | ⬜ pending |
| 01-03-T2 | 01-03 | 2 | SEC-01 | T-01-11..T-01-13 | bilingual checklist with verbatim probes + all trapdoors | smoke | 12 numbered sections; ≥12 TR: lines; ≥8 curl probes; every RESEARCH endpoint verbatim-present (grep -F loop, review MED-1); key-shape grep = 0 | ❌ W0 | ⬜ pending |
...
| 01-04-T2 | 01-04 | 3 | SEC-01 | T-01-14..T-01-16 | API keys + local rotated; CRED-17 carries structured grace= field; ATT rows filled (CEO) | manual-only (CEO probes) | gated by 01-04-T3 | n/a | ⬜ pending |
| 01-04-T3 | 01-04 | 3 | SEC-01 | T-01-14, T-01-15 | zero unresolved rows via DEAD?-column parse (review HIGH-2); ATT table complete (HIGH-3); grace literal (MED-2); timestamped; value-free | integration | column-aware awk on DEAD? column prints 21-rows 0-unresolved; ATT awk prints ATT-OK (ATT-01 = YES); CRED-17 grace-literal grep = 1; `gitleaks dir <evidence dir> --redact` exits 0 | ✅ (template from 01-03) | ⬜ pending |
...
| 01-05-T2 | 01-05 | 4 | SEC-04 | T-01-19, T-01-20, T-01-22 | sanitized doc clean at markdown + container level; original never committed; word-match line content only in /tmp scratch; no commits before CEO review (review HIGH-4) | integration | key-shape grep = 0; `gitleaks dir docs --redact` exits 0; odt-in-history grep = 0; word-match summary format-gate = 0 violations; sanitized md untracked pre-approval | ❌ (CEO produces in T1) | ⬜ pending |
...
| 01-06-T2 | 01-06 | 5 | SEC-01..04 | T-01-23, T-01-26 | every hard-gate criterion re-run live and passing; SEC-01 via column-aware DEAD?-column parser (review HIGH-5); A8 blocking rule applied (review HIGH-1) | integration | gate report exists; red-cross count = 0; report shows 21-rows 0-unresolved + ATT-OK; live gitleaks re-run exits 0 | ❌ W4 | ⬜ pending |
...
- Apify grace-window trap: old token stays valid 24h if grace accepted — checklist must decline grace or delay probe. CRED-17 must carry the structured literal `grace=declined` or `grace=accepted; probe_after_24h=yes` (machine-gated in 01-04-T3 and the 01-06 gate report).
