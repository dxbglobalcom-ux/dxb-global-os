...
- [Model routing hierarchy](model-routing-hierarchy.md) — v4 yetki matrisi + config backstop: Fable plan + repo'ya giren HER satırı (boilerplate dahil) BİZZAT yazar ve tüm final verdict'leri verir; otomatik subagent fan-out kapalı (`parallelization=false`, researcher/checker/verifier/convergence/auto-step toggles off, `hooks.workflow_guard=true`); Sonnet sadece açık Fable gerekçesiyle yazarlık-dışı high-effort destek; Haiku SADECE getir-götür; Checker PASS ≠ bitti; bütçe-fallback: Fable MASTER-PLAN + Opus 4.8 executor
...
- [CEO delegation rule](ceo-delegation-rule.md) — CEO does ONLY credential/identity steps; Claude installs/configures everything else itself, never sends setup work back to CEO
